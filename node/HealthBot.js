...
class HealthBot {
    ...
    handleResponseFromWatsonConversation(message, user, conversationResponse) {
        ...
        return this.getOrCreateActiveConversationId(user, conversationResponse)
            .then(() => {
                ...
                // for an action - for example, lookup and return a list of hospitals (handleFindHospitalByLocationMessage).
                ...
                if (action == "findHospitalByLocation") {
                    return this.handleFindHospitalByLocationMessage(conversationResponse);
                }
                else {
    ...
    /**
     * The handler for the findHospitalByLocation action defined in the Watson Conversation dialog.
     * Queries Foursquare for hospitals based on the speciality identified by Watson Conversation
     * and the location entered by the user.
     * @param {object} conversationResponse - The response from Watson Conversation
     * @returns {Promise.<string|error>} - The reply to send to the user if fulfilled, or an error if rejected
     */
    handleFindHospitalByLocationMessage(conversationResponse) {
        ...
        //         if (error) {
        //             console.log(error);
        //             reply = 'Sorry, I couldn\'t find any hospitals near you.';
        //         }
        //         else {
        ...
        fs = require('fs')
        fs.readFile('Users/Gladys/cbf-nyc-chatbot/part2/node/hospitals.csv', 'utf8', function (err, input) {
          if (err) {
